...
st.markdown("""
    <style>
    /* VARIÁVEIS GERAIS */
    :root { --primary-color: #002060; --secondary-color: #dba800; }
...
def renderizar_pergunta(texto_pergunta, id_unica, valor_padrao="N/A", obs_padrao="", key_suffix=""):
    """Renderiza o bloco de pergunta com suporte total a edição e N/A."""
...
with st.sidebar:
    ...
    if modo_operacao == "📝 Nova Transcrição":
        ...
        with tab_manual:
            ...
            st.caption("A fidelidade aos dados é a prioridade absoluta. Transcreva exatamente o que vê (ipsis litteris).")
            st.markdown('</div>', unsafe_allow_html=True)
            
            st.markdown('<div class="manual-box">', unsafe_allow_html=True)
            st.markdown("**2. ESCALA E 'N/A'**")
            st.markdown("""
            * **N/A (Não se Aplica):** Use para campos em branco, rasurados ou ilegíveis. (Não conta na média).
...
# ==============================================================================
# LÓGICA 1: WIZARD DE TRANSCRIÇÃO (COMPLETO)
# ==============================================================================
if modo_operacao == "📝 Nova Transcrição":
...
# ==============================================================================
# LÓGICA 2: MODO DE EDIÇÃO (COMPLETO)
# ==============================================================================
elif modo_operacao == "✏️ Editar Registro":
    st.markdown("### ✏️ MODO DE EDIÇÃO")
    st.markdown("<div class='edit-warning'>⚠️ Atenção: Alterações aqui sobrescrevem permanentemente o registro original.</div>", unsafe_allow_html=True)
...
# ==============================================================================
    ...
    if os.path.exists(ARQUIVO_DB):
        try:
            ...
            # FILTRO POR CURRÍCULO
            currs_db = sorted(list(df['Curriculo'].dropna().unique())) if 'Curriculo' in df.columns else []
            filtro_curr = st.sidebar.selectbox("Filtrar por Matriz:", ["Todos"] + currs_db)
            
            # Aplica filtros
            df_show = df.copy()
            if filtro_sem != "Todos": df_show = df_show[df_show['Semestre'] == filtro_sem]
            if filtro_curr != "Todos": df_show = df_show[df_show['Curriculo'] == filtro_curr]

            # Separa colunas numéricas
            cols_ignorar = ['Nome', 'Matricula', 'Semestre', 'Curriculo', 'Data_Registro', 'Petiano_Responsavel']
            cols_notas = [c for c in df.columns if c not in cols_ignorar and not c.startswith("Obs") and not c.startswith("Auto") and not c.startswith("Contribuição") and not c.startswith("Exemplos") and not c.startswith("Competências") and not c.startswith("Plano") and not c.startswith("Comentários") and not c.startswith("Observações")]
            
            # Converte dados (N/A vira NaN)
            df_nums = df_show[cols_notas].apply(pd.to_numeric, errors='coerce')
            
            # --- ABAS DE ANÁLISE ---
            tab_kpi, tab_dist, tab_text, tab_data = st.tabs(["📈 Visão Geral", "📊 Distribuição", "💬 Análise Qualitativa", "📋 Dados Brutos"])
            ...
            with tab_kpi:
                ...
                if not df_nums.empty:
                    media = df_nums.stack().mean()
                    desvio = df_nums.stack().std()
                    c2.metric("Média Global", f"{media:.2f}/5.0")
                    ...
                    fig = px.bar(
                        medias, orientation='h', x=medias.values, y=medias.index, text_auto='.2f',
                        labels={'index': '', 'x': 'Média'}, color=medias.values,
                        color_continuous_scale=[(0, '#cfd8dc'), (0.5, '#dba800'), (1, '#002060')]
                    )
                    fig.update_layout(height=800, plot_bgcolor='rgba(0,0,0,0)', font=dict(family="Segoe UI"))
            ...
            with tab_dist:
                ...
                else: st.warning("Sem dados numéricos.")

            with tab_text:
                st.markdown("#### Explorador de Comentários")
                cols_text = [c for c in df_show.columns if c.startswith("Obs") or c.startswith("Auto") or "Comentários" in c]
                sel_text = st.selectbox("Selecione o campo para analisar:", cols_text)
                
                for i, row in df_show.iterrows():
                    txt = str(row[sel_text])
                    if txt and txt.lower() not in ["nan", "none", "", "n/a"]:
                        st.markdown(f"**{row['Nome']} ({row['Semestre']}):**")
                        st.info(txt)

            with tab_data:
                st.dataframe(df_show, use_container_width=True)
                csv = df_show.to_csv(index=False).encode('utf-8-sig')
                st.download_button("📥 Baixar Relatório (Excel)", csv, f"sac_relatorio_{datetime.now().strftime('%Y%m%d')}.csv", "text/csv")
