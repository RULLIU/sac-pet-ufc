...
if modo_operacao == "📝 Nova Transcrição":
    ...
    # --- SEÇÃO 5 ---
        ...
        with st.expander("GESTÃO, ECONOMIA E MEIO AMBIENTE", expanded=True):
            ...
            renderizar_pergunta("38. Eng. Econômica: Visão global", "econ_38", key_suffix=k_suffimport streamlit as st
import pandas as pd
import os
import json
from datetime import datetime, timedelta, timezone
...
st.markdown("""
    ...
    @media (prefers-color-scheme: dark) {
        ...
        .pergunta-card { background-color: #1e1e1e !important; border-left: 5px solid #82b1ff !important; border: 1px solid #333 !important; }
        .manual-box { background-color: #262730 !important; border: 1px solid #444 !important; }
        .edit-warning { background-color: #3e2c00; color: #ffd54f; border: 1px solid #ffca28; }
    }
    ...
    @media (prefers-color-scheme: light) {
        ...
        .pergunta-card { background-color: #fcfcfc !important; border-left: 5px solid #002060 !important; border: 1px solid #e0e0e0 !important; }
        .manual-box { background-color: #f0f2f6 !important; border: 1px solid #ddd !important; }
        .edit-warning { background-color: #fff3cd; color: #856404; border: 1px solid #ffeeba; }
    }
...
if modo_operacao == "📝 Nova Transcrição":
    ...
    if secao_ativa == SECOES[0]:
        ...
        renderizar_pergunta("8. Ética profissional", "q8", key_suffix=k_suffix)
    ...
    # --- SEÇÃO 2 ---
        ...
        st.markdown("### 2. COMPETÊNCIAS ESPECÍFICAS")
        renderizar_pergunta("9. Aplicar conhecimentos matemáticos e tecnológicos", "q9", key_suffix=k_suffix)
        renderizar_pergunta("10. Modelagem de transporte (calor/massa)", "q10", key_suffix=k_suffix)
        renderizar_pergunta("11. Aplicar FT em projetos", "q11", key_suffix=k_suffix)
        renderizar_pergunta("12. Transformação da matéria e energia", "q12", key_suffix=k_suffix)
        renderizar_pergunta("13. Projetar separação/purificação", "q13", key_suffix=k_suffix)
        renderizar_pergunta("14. Cinética de reações", "q14", key_suffix=k_suffix)
        renderizar_pergunta("15. Projetar reatores", "q15", key_suffix=k_suffix)
        renderizar_pergunta("16. Controle de processos", "q16", key_suffix=k_suffix)
        renderizar_pergunta("17. Projetar plantas industriais", "q17", key_suffix=k_suffix)
        st.markdown("#### Eixos Práticos")
        renderizar_pergunta("18. Projeto básico e dimensionamento", "q18", key_suffix=k_suffix)
        renderizar_pergunta("19. Execução de melhorias de processos", "q19", key_suffix=k_suffix)
    ...
    # --- SEÇÃO 3 ---
        ...
        with st.expander("CÁLCULO E FÍSICA", expanded=True):
            renderizar_pergunta("21. Cálculo: Analisar dados", "calc_21", key_suffix=k_suffix)
            renderizar_pergunta("52. Cálculo: Formação Básica", "calc_52", key_suffix=k_suffix)
            renderizar_pergunta("22. Física: Operação de sistemas", "fis_22", key_suffix=k_suffix)
            renderizar_pergunta("53. Física: Ciência da Engenharia", "fis_53", key_suffix=k_suffix)
        with st.expander("QUÍMICA E TERMO", expanded=True):
            renderizar_pergunta("23. Química: Conhecimentos de transf.", "qui_23", key_suffix=k_suffix)
            renderizar_pergunta("24. Química: Conceber produtos", "qui_24", key_suffix=k_suffix)
            renderizar_pergunta("25. Termo: Sistemas energéticos", "termo_25", key_suffix=k_suffix)
            renderizar_pergunta("54. Termo: Ciência da EQ", "termo_54", key_suffix=k_suffix)
        with st.expander("FENÔMENOS DE TRANSPORTE", expanded=True):
            renderizar_pergunta("26. FT: Aplicar conhecimentos", "ft_26", key_suffix=k_suffix)
            renderizar_pergunta("27. FT: Comunicação gráfica", "ft_27", key_suffix=k_suffix)
            renderizar_pergunta("28. MecFlu: Implantar soluções", "mecflu_28", key_suffix=k_suffix)
            renderizar_pergunta("29. MecFlu: Supervisão", "mecflu_29", key_suffix=k_suffix)
    ...
    # --- SEÇÃO 4 ---
        ...
        with st.expander("OPERAÇÕES UNITÁRIAS", expanded=True):
            ...
            renderizar_pergunta("55. Tecnologia Industrial", "op1_55", key_suffix=k_suffix)
            renderizar_pergunta("31. Estudos ambientais", "op2_31", key_suffix=k_suffix)
            renderizar_pergunta("32. Tratamento ambiental", "op2_32", key_suffix=k_suffix)
        with st.expander("REATORES QUÍMICOS", expanded=True):
    ...
    # --- SEÇÃO 5 ---
        ...
        with st.expander("PRÁTICAS (LAB/ESTÁGIO)", expanded=True):
            renderizar_pergunta("45. Lab: Criatividade", "lab_45", key_suffix=k_suffix)
            renderizar_pergunta("46. Lab: Relacionamento", "lab_46", key_suffix=k_suffix)
            renderizar_pergunta("47. Estágio: Autocontrole", "est_47", key_suffix=k_suffix)
            renderizar_pergunta("48. Estágio: Empreendedorismo", "est_48", key_suffix=k_suffix)
    ...
    # --- SEÇÃO 6: REFLEXÃO FINAL ---
        ...
        if st.button("💾 FINALIZAR E SALVAR REGISTRO", type="primary"):
            ...
            # Varredura de Session State para notas e obs
...
# ==============================================================================
# LÓGICA 2: MODO DE EDIÇÃO (CORRIGIDO)
# ==============================================================================
    ...
    if not os.path.exists(ARQUIVO_DB):
        ...
        try:
            ...
            aluno_sel = st.selectbox("Selecione o registro para corrigir:", opcoes_alunos)
            ...
            with st.form("form_edicao_completa"):
                ...
                # Campos editáveis com valores pré-carregados
                new_nome = c1.text_input("Nome", value=dados.get("Nome", ""))
                new_mat = c2.text_input("Matrícula", value=dados.get("Matricula", ""))
                
                # Lógica para index do selectbox
                lista_sems = [f"{i}º Semestre" for i in range(1, 11)]
                curr_sem = dados.get("Semestre", "")
                idx_sem = lista_sems.index(curr_sem) if curr_sem in lista_sems else 0
                new_sem = c1.selectbox("Semestre", lista_sems, index=idx_sem)
                
                # Lógica para index do radio
                lista_curr = ["Novo (2023.1)", "Antigo (2005.1)"]
                curr_curr = dados.get("Curriculo", "")
                idx_curr = lista_curr.index(curr_curr) if curr_curr in lista_curr else 0
                new_curr = c2.radio("Currículo", lista_curr, index=idx_curr)
                
                # Petiano
                lista_pets = sorted(["", "Ana Carolina", "Ana Clara", "Ana Júlia", "Eric Rullian", "Gildelandio Junior", "Lucas Mossmann (trainee)", "Pedro Paulo"])
                curr_pet = dados.get("Petiano_Responsavel", "")
                idx_pet = lista_pets.index(curr_pet) if curr_pet in lista_pets else 0
                new_pet = st.selectbox("Responsável pela Transcrição", lista_pets, index=idx_pet)
